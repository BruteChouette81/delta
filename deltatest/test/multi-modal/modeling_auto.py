...
class DecoderConditionnalBlock(layers.Layer):
    ...
    def call(self, inputs, encoder_outputs, condition_emb, mask=None): # =None
        ...
        ### attetion layer 2
        conditionnal_tv = layers.Concatenate()[encoder_outputs, condition_emb]
...
def create_model():
    ...
    decoder = keras.Model([decoder_inputs, encoded_seq_inputs], decoder_outputs, name="output")

    decoder_outputs = decoder([decoder_inputs, encoder_outputs])
    transformer = keras.Model(
        [encoder_inputs, decoder_inputs], decoder_outputs, name="model3"
    )
    return transformer
...
auto_enc.compile("adam", loss="sparse_categorical_crossentropy", metrics=["accuracy"])
train_dataset, vectorizer = load_autoenc_vectorizer()
auto_enc.fit(train_dataset, epochs=1)
